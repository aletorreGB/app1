...
/* harmony import */ var _ionic_angular__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! @ionic/angular */ 3819);
/* harmony import */ var _services_dbservice_service__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../services/dbservice.service */ 8351);
...
let HomePage = class HomePage {
  constructor(toastController, dbtaskService, alertController, router, storage, authenticationSerive) {
  ...
  validateModel(model) {
    ...
    for (var [key, value] of Object.entries(model)) {
      ...
      if (value == "") {
        ...
        this.field = key; // Se retorna false
  ...
  presentToast(message, duration) {
    ...
    return (0,C_Users_alexa_OneDrive_Escritorio_myapp_master_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* () {
  ...
  ionViewWillEnter() {
    console.log('ionViewDidEnter'); // Se valida que exista una sesión activa

    this.dbtaskService.sesionActive().then(data => {
      if (data != undefined) {
    ...
    return (0,C_Users_alexa_OneDrive_Escritorio_myapp_master_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* () {
      const alert = yield _this2.alertController.create({
        ...
        buttons: [{
          ...
          text: 'SI',
          handler: () => {
            _this2.createSesionData(_this2.login);
          }
        }]
      });
      yield alert.present();
...
HomePage.ctorParameters = () => [{
  ...
  type: _services_dbservice_service__WEBPACK_IMPORTED_MODULE_3__.DbserviceService
}, {
  ...
  type: _services_autenthication_service__WEBPACK_IMPORTED_MODULE_4__.AutenthicationService
}];
...
/* harmony import */ var _dbservice_service__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./dbservice.service */ 8351);
...
let AutenthicationService = class AutenthicationService {
  constructor(router, storage, dbtaskService, toastController) {
    this.router = router;
    this.storage = storage;
    this.dbtaskService = dbtaskService;
    this.toastController = toastController; // Se crea un sujeto al cual van a observar y el que se actualizara con un valor booleano
  ...
  logout() {
    ...
    this.storage.get("USER_DATA").then(data => {
      ...
      data.active = 0; // Se usa la función del servicio "updateSesionData"

      this.dbtaskService.updateSesionData(data) // Manejamos la respuesta correcta
      .then(response => {
        // Se valida que efectuo una modificación en alguna fila
        if (response.rowsAffected >= 1) {
          // Se remueve el valor de USER_DATA
          this.storage.remove("USER_DATA"); // Se rederige al login
  ...
  login(login) {
    // Se obtiene si existe alguna data de sesión
    this.dbtaskService.getSesionData(login).then(data => {
      if (data === undefined) {
        // Si es undefined es por que no retorno firmas
        this.presentToast("Credenciales Incorrectas");
      } else {
        // Si no es undefined es por que el usuario y la password coincidieron con algun registro
...
AutenthicationService = (0,tslib__WEBPACK_IMPORTED_MODULE_6__.__decorate)([(0,_angular_core__WEBPACK_IMPORTED_MODULE_7__.Injectable)({
  providedIn: 'root'
...
DbserviceService = (0,tslib__WEBPACK_IMPORTED_MODULE_0__.__decorate)([
